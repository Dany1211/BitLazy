...
import { Profile } from '@/lib/analytics'
    ...
    return (
        <div className="h-screen bg-[#F8FAFC] flex flex-col font-sans antialiased overflow-hidden">
                ...
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-sm font-black text-[#0F172A] truncate max-w-[120px] sm:max-w-xs tracking-tight uppercase">
                                {session.title}
                            </h1>
                        </div>
                    </div>
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="hidden sm:flex items-center gap-2">
                        <div className="flex -space-x-2">
                            {allProfiles?.slice(0, 3).map((p: Profile) => (
                                <div key={p.id} title={p.name || 'User'} className="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-400 uppercase overflow-hidden ring-1 ring-slate-100">
                                    {p.avatar_url ? (
                                        <img src={p.avatar_url} alt={p.name || 'Avatar'} className="w-full h-full object-cover" />
                                    ) : (
            ...
            <div className="flex-1 flex overflow-hidden">
                        ...
                        <section>
                            ...
                            <div className="space-y-4">
                                ...
                                <Link
                                    ...
                                    className="flex justify-between items-center bg-emerald-500/10 p-3 rounded-xl border border-emerald-500/20 hover:bg-emerald-500/20 transition-all group"
                                >
                                    <span className="text-[11px] font-bold text-emerald-400">View Analytics</span>
                                    <svg className="w-3 h-3 text-emerald-400 group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {allProfiles?.map((p: Profile) => (
                                    <div key={p.id} className="flex items-center gap-3 group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-all border border-transparent hover:border-white/10">
                                        <div className="w-8 h-8 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 font-black text-[10px] uppercase overflow-hidden">
                                            {p.avatar_url ? <img src={p.avatar_url} className="w-full h-full object-cover" alt={p.name || 'Avatar'} /> : (p.name?.charAt(0) || 'U')}
                                        </div>
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tight">{p.id === user.id ? 'You' : 'Contributor'}</span>
                                        </div>
                ...
                {/* Right Pane: Session Synthesis - Persistent in layout */}
